      ...
      <div class="flex-1 flex items-center justify-center px-6 pb-12">
        <div class="w-full max-w-md">
          ...
          <p class="text-center text-gray-600 mt-8 text-sm">
            ...
            <NuxtLink to="/signup" class="text-[#1e3a5f] font-medium hover:underline">Signup</NuxtLink>
          </p>
        </div>
      </div>
    </div>
...
const { login: doLogin, isAdmin } = useAuth()
...
function onSubmit() {
  loginError.value = ''
  const result = doLogin(form.value.email, form.value.password)
  if (!result.success) {
    loginError.value = result.error ?? 'Invalid email or password'
    return
  }
  if (form.value.rememberMe && import.meta.client && typeof localStorage !== 'undefined') {
    localStorage.setItem('kathies-remember-email', form.value.email)
  }
...
function signInWithGoogle() {
  // Placeholder: wire to OAuth when backend is ready
  navigateTo('/')
}

onMounted(() => {
  if (import.meta.client && typeof localStorage !== 'undefined') {
    const saved = localStorage.getItem('kathies-remember-email')
    if (saved) form.value.email = saved
  }
})

useHead({ title: "Sign In | Kathie's Kitchen" })
</script>
